...
import { updateBeltSystem } from "../../../actions";

export function EditBeltSystemForm({
  id,
  modalities,
  defaultValues,
}: {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<BeltSystemInput>({
    resolver: zodResolver(beltSystemSchema),
    defaultValues,
  });

  async function onSubmit(data: BeltSystemInput) {
    setIsSubmitting(true);
    const result = await updateBeltSystem(id, data);
    ...
    toast.success("Sistema de faixa atualizado.");
    router.push("/belts");
  }

...
import { createBelt } from "../../../../actions";

export function NewBeltForm({ beltSystemId }: { beltSystemId: string }) {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<BeltInput>({ resolver: zodResolver(beltSchema) });

  async function onSubmit(data: BeltInput) {
    setIsSubmitting(true);
    const result = await createBelt(beltSystemId, data);
  ...
  return (
    ...
    >
      ...
      <div className="space-y-1.5">
        ...
        <Input id="colorHex" placeholder="#000000" {...register("colorHex")} />
      </div>
      ...
      <Button type="submit" className="w-full" disabled={isSubmitting}>
        {isSubmitting ? "Criando..." : "Criar faixa"}
      </Button>
    </form>
  );
}
